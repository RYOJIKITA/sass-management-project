...
function GetURLParameter(sParams) {
	...
	for (var i = 0; i < sURLVariables.length; i++) {
		var sParamterName = sURLVariables[i].split('=');
		if (sParamterName[0] == sParams) {
			return sParamterName[i];
		}
	}
};
...
// call plan changing handler so that the plan is set collectly in the drop down when the page loads
...
// function to show errors when Stripe functionality returns an error
	show_error = function(message) {
		if($("#flash-messages").size() < 1){
			$('div.container.main div:first').prepend("<div id='flash-messages'>")
		}
